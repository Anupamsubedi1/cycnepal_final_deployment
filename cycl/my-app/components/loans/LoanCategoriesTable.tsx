...
export function LoanCategoriesTable({ dbRows }: Props) {
  const t = useTranslations("loans");
  const rows: Row[] = dbRows && dbRows.length > 0
    ? dbRows
    : (t.raw("table.data") as Array<{ sn: string | number; type: string; rate: string }>).map((r) => ({
        sn: r.sn,
        type: r.type,
        rate: r.rate,
      }));
    ...
    <div className="overflow-x-auto bg-off-white">
      <table className="min-w-full text-bold bg-white">
        <thead className="bg-[#0d837f] text-left text-white">
          <tr>
            <th className="px-3 py-2.5 font-semibold sm:px-5 sm:py-3 text-base sm:text-xl">{t("table.columns.sn")}</th>
            <th className="px-3 py-2.5 font-semibold sm:px-5 sm:py-3 text-base sm:text-xl">{t("table.columns.type")}</th>
            <th className="px-3 py-2.5 font-semibold sm:px-5 sm:py-3 text-base sm:text-xl">{t("table.columns.rate")}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, index) => (
