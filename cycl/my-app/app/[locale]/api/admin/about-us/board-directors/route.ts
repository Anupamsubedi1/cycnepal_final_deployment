...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await createBoardDirector({
      ...
      isChairman: Boolean(body.isChairman),
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const result = await updateBoardDirector(id, {
      "name-en": body["name-en"],
      "name-ne": body["name-ne"] || body["name-en"],
      "role-en": body["role-en"],
      "role-ne": body["role-ne"] || body["role-en"],
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const director = directors.find((d) => d._id?.toString() === id);
    if (director?.imagePublicId) {
      try { await deleteCloudinaryImage(director.imagePublicId); } catch {}
    }

    const deleted = await deleteBoardDirector(id);
