...
import type { BoardPageSettings, BoardDirector } from "@/services/board-of-directors-service";
import HeroBannerUpload from "@/components/admin/HeroBannerUpload";
import {
  ChevronRight, RefreshCw, Save, UserCircle2, Image as ImageIcon,
  UploadCloud, Eye, Plus, Trash2, Edit3, X, Users, Star,
} from "lucide-react";

function createEmptySettings(): Omit<BoardPageSettings, "_id" | "createdAt" | "updatedAt"> {
  return {
...
function createEmptyMember(): Omit<BoardDirector, "_id" | "createdAt" | "updatedAt"> {
  return {
    "name-en": "", "name-ne": "",
    "role-en": "", "role-ne": "",
    phone: "", email: "", address: "",
    imageUrl: "", imagePublicId: "",
    isChairman: false, order: 0,
  };
}

export default function BoardOfDirectorsManagement() {
  ...
  const [members, setMembers] = useState<BoardDirector[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      const [settingsRes, membersRes] = await Promise.all([
        fetch("/api/admin/about-us/board-page-settings"),
        fetch("/api/admin/about-us/board-directors"),
      ]);
      ...
      setError("Failed to load board of directors data");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const res = await fetch("/api/admin/about-us/board-page-settings", {
        method: "POST",
  ...
  const handleSaveMember = async () => {
    ...
    try {
      const url = editingMemberId
        ? `/api/admin/about-us/board-directors?id=${editingMemberId}`
        : "/api/admin/about-us/board-directors";
      ...
      if (!res.ok) throw new Error("Failed to save");
      setSuccess(editingMemberId ? "Director updated!" : "Director added!");
  ...
  const handleEditMember = (member: BoardDirector) => {
  ...
  const handleDeleteMember = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/about-us/board-directors?id=${id}`, { method: "DELETE" });
  ...
  if (loading) return <div className="p-12 text-slate-400 animate-pulse font-medium">Loading Board of Directors Workspace...</div>;
    ...
    <div className="min-h-screen bg-[#F0F2F5] text-[#334155] font-sans pb-20">
      <header className="sticky top-0 z-10 bg-white border-b border-slate-200 px-8 py-3 flex items-center justify-between">
        <nav className="flex items-center text-sm font-medium text-slate-500">
          ...
          <span className="text-slate-900 font-semibold">Board of Directors</span>
        </nav>
      ...
      <main className="max-w-7xl mx-auto mt-8 px-6 space-y-10">
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-bold uppercase tracking-widest text-slate-400 flex items-center gap-2">
              <Users size={14} className="text-teal-500" /> Directors ({members.length})
            </h2>
            <button
              ...
              className="bg-[#40C9C0] hover:bg-[#34b1a9] disabled:opacity-50 text-white px-5 py-2 rounded-md font-semibold text-sm transition-all flex items-center gap-2 shadow-sm cursor-pointer"
            >
              <Save size={14} /> {savingMember ? "SAVING..." : (editingMemberId ? "UPDATE DIRECTOR" : "ADD DIRECTOR")}
            </button>
          </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">{editingMemberId ? "Edit Director" : "Add Director"}</h3>
                  </div>
                  {editingMemberId && (
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="flex items-center gap-3 pt-5">
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" checked={memberForm.isChairman} onChange={(e) => setMemberForm(p => ({ ...p, isChairman: e.target.checked }))} className="sr-only peer" />
                        <div className="w-10 h-6 bg-slate-200 rounded-full peer peer-checked:bg-[#40C9C0] transition-colors" />
                        <div className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-4" />
                      </label>
                      <span className="text-sm font-semibold text-slate-600 flex items-center gap-1"><Star size={13} className="text-amber-400" /> Is Chairman</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 flex items-center gap-2">
                  <ImageIcon size={14} /> Director Photo
                </h3>
                <div className="relative group rounded-lg overflow-hidden bg-slate-100 aspect-4/5 border-2 border-dashed border-slate-200 flex items-center justify-center">
                  {memberForm.imageUrl ? (
                    <>
                      <Image src={memberForm.imageUrl} alt="Director" fill className="object-cover" />
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-slate-100 bg-[#F8FAFC] flex items-center justify-between">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Current Directors</h3>
                  <span className="text-xs text-slate-400">{members.length} total</span>
                </div>
                <div className="divide-y divide-slate-50 max-h-[400px] overflow-y-auto">
                  {members.length === 0 ? (
                    <p className="p-6 text-xs text-slate-400 italic text-center">No directors added yet.</p>
                  ) : (
                    members.map((m) => (
                        ...
                        <div className="flex gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-1.5">
                              ...
                              {m.isChairman && <Star size={10} className="text-amber-400 shrink-0" />}
                            </div>
